...
void MainWindow::createHeader()
{
    ...
    // Name widget
    QLabel* nameWidget = new QLabel("NanoInference");
...
void MainWindow::createChatArea()
{
    ...
    messageInput->setMaximumHeight(50);
    messageInput->setPlaceholderText("Type your message here...");
    
    sendButton = new QPushButton("Send");  // Add text or icon
    sendButton->setFixedSize(50, 50);
    sendButton->setObjectName("sendButton");
    
    inputLayout->addWidget(messageInput);
    inputLayout->addWidget(sendButton);
    inputContainer->setLayout(inputLayout);  // Set the layout explicitly
    
    chatLayout->addWidget(chatDisplay, 1);  // Give chat display a stretch factor
    chatLayout->addWidget(inputContainer, 0);  // Fixed size for input container
}
...
void MainWindow::onSendClicked() {
    ...
    chatDisplay->append("<strong>You:</strong> " + message);
    messageInput->clear();
    ...
    connect(watcher, &QFutureWatcher<QString>::finished, this, [this, watcher]() {
        ...
        // Add model response
        chatDisplay->append("<strong>AI:</strong> " + response);
